...
def init_db():
    """DB 파일이 없으면 생성하고 스키마를 초기화한다. 신규 컬럼은 마이그레이션으로 추가."""
...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    cases_additions = {
        ...
        "피신청인2_성명":     "ALTER TABLE cases ADD COLUMN 피신청인2_성명 TEXT",
        "피신청인2_지위":     "ALTER TABLE cases ADD COLUMN 피신청인2_지위 TEXT",
        "피신청인2_주소":     "ALTER TABLE cases ADD COLUMN 피신청인2_주소 TEXT",
        "피신청인2_우편번호": "ALTER TABLE cases ADD COLUMN 피신청인2_우편번호 TEXT",
        "피신청인2_연락처":   "ALTER TABLE cases ADD COLUMN 피신청인2_연락처 TEXT",
    }
...
def get_all_cases(year: int | None = None, status: str | None = None) -> list[sqlite3.Row]:
    ...
    sql += " ORDER BY 접수연도 DESC, CAST(SUBSTR(접수번호, INSTR(접수번호, '-') + 1) AS INTEGER) DESC"
    with get_conn() as conn:
...
def delete_case(접수번호: str) -> None:
    with conn_ctx() as conn:
        conn.execute("DELETE FROM case_notes WHERE 접수번호 = ?", (접수번호,))
        conn.execute("DELETE FROM hearings WHERE 접수번호 = ?", (접수번호,))
...
def get_stats_by_period(start_date: str, end_date: str, date_col: str = "접수일자") -> dict:
    ...
    with get_conn() as conn:
        ...
        closed = conn.execute(
            f"""SELECT COUNT(*) FROM cases {base} AND (
                결과 IN ('조정성립','조정불성립','조정중지','종결')
                OR 개최여부 = '불개시'
                OR 조정동의여부 = '부동의'
            )""", p
        ).fetchone()[0]
...
def get_monthly_counts(year: int) -> list[dict]:
    """해당 연도의 월별 접수·종결 건수"""
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT
                CAST(strftime('%m', 접수일자) AS INTEGER) AS 월,
                COUNT(*) AS 접수,
                SUM(CASE WHEN
                    결과 IN ('조정성립','조정불성립','종결','조정중지')
                    OR 개최여부 = '불개시'
                    OR 조정동의여부 = '부동의'
...
def get_deadline_cases(days: int = 7) -> list[sqlite3.Row]:
    """오늘부터 days일 이내 회신기한인 미종결 사건"""
    ...
    with get_conn() as conn:
        return conn.execute("""
            SELECT * FROM cases
            WHERE 회신기한 BETWEEN ? AND ?
              AND (결과 IS NULL OR 결과 NOT IN ('조정성립','조정불성립','종결','조정중지'))
              AND (개최여부 IS NULL OR 개최여부 != '불개시')
              AND (조정동의여부 IS NULL OR 조정동의여부 != '부동의')
            ORDER BY 회신기한
